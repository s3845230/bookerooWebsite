...
public class BookControllerTest {
    ...
    public void createBook_success() throws Exception {
        System.out.println("test createBook_success");
    ...
    public void getBookbySearch_success() throws Exception {
        System.out.println("test getBookbySearch_success");
    ...
    public void updateBook_success() throws Exception {
        ...
        System.out.println("test updateBook_success");
        //intercept from entering database
        Mockito.when(bookRepository.findById(book1.getId())).thenReturn(Optional.of(book1));
        Mockito.when(bookRepository.save(book1)).thenReturn(book1);

        //create mock api post request
        MockHttpServletRequestBuilder mockPutRequest = MockMvcRequestBuilders.put("/api/book/update")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .content("{\"id\": \"1\"," +
                        "\"isbn\": \"isbn\"," +
        ...
        //perform request and check title is correct
        mockMvc.perform(mockPutRequest)
                .andExpect(status().is2xxSuccessful())
                .andExpect(jsonPath("$", notNullValue()))
                .andExpect(jsonPath("$.title", is("title")));
    }
...
//                .andExpect(result ->
//                        Assertions.assertEquals("id must not be null!", result.getResolvedException().getMessage()));
//    }
